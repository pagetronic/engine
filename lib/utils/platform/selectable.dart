...
///
/// Used by [Selectable.searchCallback].
typedef SearchCallback<T> = int? Function(List<SelectableEntry<T>> entries, String query);
...
/// Defines a [Selectable] menu button that represents one item view in the menu.
///
/// See also:
///
/// * [Selectable]
class SelectableEntry<T> {
  /// Creates an entry that is used with [Selectable.selectableEntries].
  ...
  ///
  /// This value must be unique across all entries in a [Selectable].
  final T value;
  ...
  ///
  /// The default value is true. If true, the [SelectableEntry.label] will be filled
  /// out in the text field of the [Selectable] when this entry is clicked; otherwise,
...
///
/// The menu is composed of a list of [SelectableEntry]s. People can provide information,
...
///
/// {@tool dartpad}
/// This sample shows how to display outlined [Selectable] and filled [Selectable].
///
...
///   The [Selectable] uses a [TextField] as the "anchor".
/// * [TextField], which is a text input widget that uses an [InputDecoration].
/// * [SelectableEntry], which is used to build the [MenuItemButton] in the [Selectable] list.
class Selectable<T> extends StatefulWidget {
  /// Creates a const [Selectable].
  ///
  ...
  const Selectable({
    ...
    this.expandedInsets,
    this.searchCallback,
    required this.selectableEntries,
    this.inputFormatters,
  });

  /// Determine if the [Selectable] is enabled.
  ///
  /// Defaults to true.
  final bool enabled;

  /// Determine the width of the [Selectable].
  ///
  /// If this is null, the width of the [Selectable] will be the same as the width of the widest
  ...
  /// Text that provides context about the [Selectable]'s value, such
  ...
  /// The text style for the [TextField] of the [Selectable];
  ///
  ...
  /// Descriptions of the menu items in the [Selectable].
  ///
  /// This is a required parameter. It is recommended that at least one [SelectableEntry]
  /// is provided. If this is an empty list, the menu will be empty and only
  /// contain space for padding.
  final List<SelectableEntry<T>> selectableEntries;
  ...
  /// by the width of menu items or [Selectable.width]. If this property is not null,
  ...
  /// When  [Selectable.enableSearch] is true, this callback is used to compute
  ...
  /// ```dart
  /// Selectable<Text>(
  ///   searchCallback: (List<SelectableEntry<Text>> entries, String query) {
  ///     if (query.isEmpty) {
  ///       return null;
  ///     }
  ///     final int index = entries.indexWhere((SelectableEntry<Text> entry) => entry.label == query);
  ///
  ///     return index != -1 ? index : null;
  ///   },
  ///   SelectableEntries: const <SelectableEntry<Text>>[],
  /// )
  /// ```
  /// {@end-tool}
  ///
  /// Defaults to null. If this is null and [Selectable.enableSearch] is true,
  ...
  @override
  State<Selectable<T>> createState() => _SelectableState<T>();
}
...
class _SelectableState<T> extends State<Selectable<T>> {
  ...
  late List<SelectableEntry<T>> filteredEntries;
  ...
  void initState() {
    ...
    filteredEntries = widget.selectableEntries;
    buttonItemKeys = List<GlobalKey>.generate(filteredEntries.length, (int index) => GlobalKey());
    _menuHasEnabledItem = filteredEntries.any((SelectableEntry<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere((SelectableEntry<T> entry) => entry.value == widget.initialSelection);
    if (index != -1) {
  ...
  @override
  void didUpdateWidget(Selectable<T> oldWidget) {
    ...
    if (oldWidget.selectableEntries != widget.selectableEntries) {
      currentHighlight = null;
      filteredEntries = widget.selectableEntries;
      buttonItemKeys = List<GlobalKey>.generate(filteredEntries.length, (int index) => GlobalKey());
      _menuHasEnabledItem = filteredEntries.any((SelectableEntry<T> entry) => entry.enabled);
    ...
    if (oldWidget.initialSelection != widget.initialSelection) {
      final int index =
          filteredEntries.indexWhere((SelectableEntry<T> entry) => entry.value == widget.initialSelection);
      if (index != -1) {
        _textEditingController.value = TextEditingValue(
          text: filteredEntries[index].label,
          selection: TextSelection.collapsed(offset: filteredEntries[index].label.length),
  ...
  void refreshLeadingPadding() {
    ...
    }, debugLabel: 'Selectable.refreshLeadingPadding');
  }
  ...
  void scrollToHighlight() {
    ...
    }, debugLabel: 'Selectable.scrollToHighlight');
  }
  ...
  List<SelectableEntry<T>> filter(List<SelectableEntry<T>> entries, TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries.where((SelectableEntry<T> entry) => entry.label.toLowerCase().contains(filterText)).toList();
  }

  int? search(List<SelectableEntry<T>> entries, TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final int index = entries.indexWhere((SelectableEntry<T> entry) => entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<SelectableEntry<T>> filteredEntries, TextDirection textDirection,
      {int? focusedIndex, bool enableScrollToHighlight = true}) {
    final List<Widget> result = <Widget>[];
    for (int i = 0; i < filteredEntries.length; i++) {
      final SelectableEntry<T> entry = filteredEntries[i];
      ...
      final ButtonStyle defaultStyle = switch (textDirection) {
        TextDirection.rtl =>
          MenuItemButton.styleFrom(padding: EdgeInsets.only(left: _kDefaultHorizontalPadding, right: padding)),
        TextDirection.ltr =>
          MenuItemButton.styleFrom(padding: EdgeInsets.only(left: padding, right: _kDefaultHorizontalPadding)),
      };
      ...
      Widget label = entry.labelWidget ?? Text(entry.label, overflow: TextOverflow.ellipsis);
  ...
  Widget build(BuildContext context) {
    ...
    _initialMenu ??= _buildButtons(widget.selectableEntries, textDirection, enableScrollToHighlight: false);
    final DropdownMenuThemeData theme = DropdownMenuTheme.of(context);
    final DropdownMenuThemeData defaults = _SelectableDefaultsM3(context);

    if (_enableFilter) {
      filteredEntries = filter(widget.selectableEntries, _textEditingController);
    }
    ...
    if (widget.width != null) {
      effectiveMenuStyle = effectiveMenuStyle.copyWith(
          maximumSize: WidgetStatePropertyAll<Size?>(Size(widget.width!, 0.0)),
          minimumSize: WidgetStatePropertyAll<Size?>(Size(widget.width!, 0.0)));
    } else if (anchorWidth != null) {
      effectiveMenuStyle = effectiveMenuStyle.copyWith(
          maximumSize: WidgetStatePropertyAll<Size?>(Size(anchorWidth, 0.0)),
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder: (BuildContext context, MenuController controller, Widget? child) {
        ...
        final Widget textField = TextField(
            ...
            onEditingComplete: () {
              if (currentHighlight != null) {
                final SelectableEntry<T> entry = filteredEntries[currentHighlight!];
                if (entry.enabled) {
                  _textEditingController.value = TextEditingValue(
                    text: entry.label,
                    selection: TextSelection.collapsed(offset: entry.label.length),
        ...
        if (widget.expandedInsets != null) {
          ...
          // on its parent width. So we don't need to use `_SelectableBody` to
          // calculate the children's width.
          return textField;
        }

        return _SelectableBody(
          width: anchorWidth ?? widget.width,
          children: <Widget>[
            textField,
...
class _SelectableBody extends MultiChildRenderObjectWidget {
  ...
  @override
  _RenderSelectableBody createRenderObject(BuildContext context) {
    return _RenderSelectableBody(
      width: width,
    );
  }

  @override
  void updateRenderObject(BuildContext context, _RenderSelectableBody renderObject) {
    renderObject.width = width;
  }
}

class _SelectableBodyParentData extends ContainerBoxParentData<RenderBox> {}

class _RenderSelectableBody extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, _SelectableBodyParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, _SelectableBodyParentData> {
  _RenderSelectableBody({
    double? width,
  }) : _width = width;
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! _SelectableBodyParentData) {
      child.parentData = _SelectableBodyParentData();
    }
  }
  ...
  void performLayout() {
    ...
    while (child != null) {
      if (child == firstChild) {
        ...
        final _SelectableBodyParentData childParentData = child.parentData! as _SelectableBodyParentData;
...
class _SelectableDefaultsM3 extends DropdownMenuThemeData {
  _SelectableDefaultsM3(this.context);
